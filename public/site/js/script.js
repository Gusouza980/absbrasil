...
>>>>>>> c0092c91a3522bf7aae6a4b7699e8e4518ac0790
  ...
  valida: (i) => {
    ...
    if (i.name == "nome") valido = ifl1.formatador.limpar(i.value).length >= 3;
      ...
      valido = /^\(?[1-9]{2}\)? ?(?:[2-8]|9[1-9])[0-9]{3}\-?[0-9]{4}$/.test(
...
<<<<<<< HEAD
    ...
    else if (i.name == "nascimento")
      valido = ifl1.formatador.soNumeros(i.value).length == 8;
        ...
        fetch(
          ...
          r.json().then((json) => {
            if (json.hasOwnProperty("state")) {
              wfform.valida(
                $(i.form).find('.input[name="uf"]').val(json.state)[0]
              );
              wfform.valida(
                $(i.form).find('.input[name="cidade"]').val(json.city)[0]
              );
              wfform.valida(
                $(i.form)
                  .find('.input[name="bairro"]')
                  .val(json.neighborhood)[0]
              );
              wfform.valida(
...
<<<<<<< HEAD
      if (
        ...
        i.value = ifl1.formatador.digitado.nome(
          ifl1.formatador.limpar(i.value)
        );
      else if (i.name == "celular" || i.name == "telefone")
        i.value = ifl1.formatador.digitado.telefone(
          ifl1.formatador.soNumeros(i.value)
        );
      else if (i.name == "cpf")
        i.value = ifl1.formatador.digitado.cpf(
          ifl1.formatador.soNumeros(i.value)
        );
      else if (i.name == "cep")
...
>>>>>>> c0092c91a3522bf7aae6a4b7699e8e4518ac0790
  ...
  envia: (btn) => {
    ...
    if ($(btn.form).find(".valido").length < $(btn.form).find("label").length) {
      ...
      fetch("/_ajax.php?cadastro", opt).then((r) =>
        r.json().then((json) => {
          btn.disabled = false;
          let aviso = $(btn.form).find(".aviso").first();
          aviso.html(json.msg);
...
<<<<<<< HEAD
  ...
  $("a.ancora").click((e) => {
    if (location.pathname == "/") {
      e.preventDefault();
      let href =
        $(e.target).attr("href") || $(e.target).closest("a").attr("href");
      if (
...
$(window).on("load", function () {
  ...
  if (pathname[pathname.length - 1] == "/")
    pathname = pathname.substr(0, pathname.length - 1); //remove "/" do fim do endereço
  if (pathname.includes("%20")) pathname = pathname.split("%20").join("-"); //troca espaços por traços
  if (pathname != location.pathname)
    window.history.replaceState({}, "", pathname); //efetiva correções, se necessário
  wfform.init();
});
...
$(".terms input").change(() => {
  $("div.terms a").toggleClass("active");
});
